...
export async function POST(request: NextRequest) {
  try {
    ...
    const { nurseId, nurseName, title, titleAr, description, descriptionAr, category, beforeImage, afterImage, videoUrl } = body;

    if (!nurseId || !title || !description || !category) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    if (action === 'verify') {
      ...
      return NextResponse.json({ success: true, message: 'Story verified!' });
    }
    ...
    if (action === 'feature') {
      ...
      return NextResponse.json({ success: true, message: 'Story featured!' });
    }
