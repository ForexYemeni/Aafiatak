...
import { getServiceRequestsByBeneficiary, createServiceRequest, getServiceById, getBeneficiaryById } from '@/lib/firestore'
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      const requestId = (serviceRequest as any)?.id || (serviceRequest as any)?._id?.toString() || ''
      notifyAdminServer.newOrder(beneficiaryName, serviceNames, requestId).catch(() => {})
    } catch {}
