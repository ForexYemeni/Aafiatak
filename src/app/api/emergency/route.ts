...
export async function POST(request: NextRequest) {
  try {
    ...
    // ─── Server-side notification: Notify all admins about the emergency request ───
    const beneficiaryName = body.beneficiaryName || 'مستفيد'
    const emergencyId = (emergencyRequest as any)?.id || (emergencyRequest as any)?._id?.toString() || ''
    notifyAdminServer.emergencyRequest(beneficiaryName, emergencyId).catch((err: any) =>
      console.error('Failed to send emergency notification to admins:', err.message)
    )
